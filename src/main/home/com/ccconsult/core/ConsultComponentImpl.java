...
import com.ccconsult.view.CounsultInterviewVO;
import com.ccconsult.view.ConsultResumeVO;
...
public class ConsultComponentImpl implements ConsultComponent {
    ...
    private ConsultBase consConsultBase(Consult consult) {
        ...
        if (consult.getServiceId() == 2) {//简历修改服务
            ...
            ((ConsultResumeVO) base).setConsultResume(resumeConsultDAO.findByConsultId(consult
                .getId()));
        }
        if (consult.getServiceId() == 4) {
            base = new CounsultInterviewVO();
            ((CounsultInterviewVO) base).setConsultInterview(interviewConsultDAO
    ...
    @Autowired
    private ConsultResumeDAO    resumeConsultDAO;
    @Autowired
    private ConsultantDAO       consultantDAO;
    @Autowired
    private ConsultInterviewDAO interviewConsultDAO;
    @Autowired
